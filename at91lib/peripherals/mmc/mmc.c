...
char mmc_buffer[BUFFER_SIZE] = { 0 };	// Buffer for mmc i/o for data and registers
	//cvm: changed to size of a char
...
void Init_CP_WP (void) {
  ...
  // pull up resistor
  //s_pSys->PIOA_PPUDR = 0xffffffff;  // Disable Pull-up resistor cvm 1/6/2010 
  
  //Write protect -> WP - PA15
  s_pPio->PIO_ODR = BIT15; //Configure in Input
  s_pPio->PIO_PER = BIT15; //Enable PA15

  //Card present -> CP - PA16
...
{
  //SPI is set to slave by default following a software reset(See datasheet pg 278)
  s_pSpi->SPI_CR      = 0x81;              //SPI Enable, Sowtware reset
  s_pSpi->SPI_CR      = 0x02;              //SPI Disable
  //s_pSpi->SPI_CR      = 0x01;              //SPI Enable
}
...
char mmcWriteBlock (const unsigned long address)
{
  ...
  // Set the block length to read
  if (mmcSetBlockLength (BUFFER_SIZE) == MMC_SUCCESS)	// block length could be set
    {
      // SS = LOW (on)
      //CS_LOW ();
      // send write command
...
#define ENABLED_SPI 0xFFFFFFFF  //((1 << 11) | (1 << 12) | (1 << 13) | (1 << 14))
#define DISABLED_SPI 0x00000000

void setHighImpedance(void) {
  //if (s_pPio->PIO_MDSR != ENABLED_32) {  // Multi-driver status register.
    s_pPio->PIO_MDER = ENABLED_SPI ;  // Multi-driver enable register.
    s_pPio->PIO_MDDR = DISABLED_SPI;  // Mulit-driver disable register.
  //}
}

void setActive(void) {
  //if (s_pPio->PIO_MDSR == ENABLED_32) {  // Multi-driver status register.
    s_pPio->PIO_MDER = DISABLED_SPI;  // Multi-driver enable register.
    s_pPio->PIO_MDDR = ENABLED_SPI;  // Mulit-driver disable register.
  //}
}
